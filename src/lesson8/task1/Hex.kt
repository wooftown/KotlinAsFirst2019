...
data class Hexagon(val center: HexPoint, val radius: Int) {
     ...
     */
    fun distance(other: Hexagon): Int = max(center.distance(other.center) - radius - other.radius, 0)
...
class HexSegment(val begin: HexPoint, val end: HexPoint) {
     ...
     */
    fun direction(): Direction = if (!isValid()) {
        Direction.INCORRECT
    } else {
        when {
            begin.y == end.y -> if (begin.x < end.x) Direction.RIGHT else Direction.LEFT
            begin.x == end.x -> if (begin.y > end.y) Direction.DOWN_LEFT else Direction.UP_RIGHT
